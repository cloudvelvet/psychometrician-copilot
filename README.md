...
연구 조건을 입력하면 분석 로드맵, 근거 토픽, 코드 템플릿, 경고를 한 화면에서 확인할 수 있습니다. 입력 화면의 `Intake Coach`는 문항 컬럼명, 결측 정보, 범주 분포, 집단 변수, 고위험 사용 여부를 보면서 다음에 채울 항목을 즉시 안내합니다.

상담 리포트에는 `Study Packet Builder`가 포함됩니다. 이 패킷은 API 없이 브라우저에서 결정론적으로 만들어지며, 연구 설계 요약, 변수 manifest, 임시 문항-요인 매핑표, 분석 순서, 누락 정보, 보고문 체크리스트, R 코드 템플릿, 다음에 입력해야 할 정보를 한 덩어리로 정리합니다.

문항 ID가 실제 데이터 컬럼으로 제공되지 않으면 `item1`, `item2` 같은 placeholder가 표시됩니다. 이 값은 실행 가능한 최종 분석표가 아니라 실제 설문지와 데이터 사전으로 교체해야 하는 템플릿입니다.
...
Study Packet Builder는 연구 계획 산출물을 만드는 기능입니다. R 코드를 실행하거나 CFA/IRT/DIF 결과를 검증하지 않으며, placeholder 문항명·요인명·역채점 정보는 실제 데이터와 연구 설계에 맞게 연구자가 확정해야 합니다.
